...
results_files_all <- list.files("pipeline_FL/FL_results/", 
                                 pattern = "uSCR_real_Augustine_Binom_Final_[0-9]_Full_Offset_.*_VPS_map.RDS",
                                 full.names = TRUE)

combos <- expand.grid(
  spatial_type  = c(#"HB_map", 
    "VPS_map"
  ),
...
for (i in 1:nrow(combos)) {
  this_resfiles <- results_files_all[grepl(paste0(combos$integration_type[i], "_", 
                                                  combos$sigma_type[i], "_",
                                                  combos$spatial_type[i]),
                                           results_files_all)]
  ...
  summary_list[[i]] <- MCMCsummary(samples_list_list[[i]]) %>%
    mutate(integration_type = combos$integration_type[i],
           ...
           spatial_type = combos$spatial_type[i]
           )
...
prior_df <- bind_rows(
  prior_df %>% mutate(spatial_type = "HB_map"),
  prior_df %>% mutate(spatial_type = "VPS_map")
)
...
plot_N <- summary_df %>% 
  ...
  ggplot() +
  geom_pointrange(aes(param, mean/2, ymin = `2.5%`/2, ymax = `97.5%`/2, col = sigma_type,
                      group = type, shape = sigma_type), position = position_dodge(width = 0.4)) +
  theme_bw() +
  scale_color_manual("Model", values = type_colors) +
...
for (i in 1:nrow(combos)) {
  ESA_list[[i]] <- calc_ESA(result_list = list(samples = samples_list_list[[i]]), distr = "Binom",
                            ...
                            esa_sigma_type = "attraction plus 20-min", attraction_dist = 27.11) %>% 
    mutate(integration_type = summary_list[[i]]$integration_type[1], 
           sigma_type = summary_list[[i]]$sigma_type[1],
           spatial_type = summary_list[[i]]$spatial_type[1],
