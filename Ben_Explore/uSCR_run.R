...
#simulate some data
N <- 30       # Abundance
lam0 <- 1  # Mean detection prob
sigma <- 0.5 # Space use parameter
K <- 5        # Replicates per camera
buff <- 6 #state space buffer. Should be at least 3 sigma.
X<- expand.grid(3:11,3:11) # Positions of cameras
obstype <- "poisson"
...
plot(mcmc(mvSamples[2:nrow(mvSamples), 1]))
